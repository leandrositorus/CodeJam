  ...
  return [
    ...
    ...(request.authorization ? [
      ...
      request.authorization.endpoint
        ? "AGENT_PASSPORT_HOST=" + request.authorization.endpoint.host
        : "AGENT_PASSPORT_SOCKET=/run/agent-passport.sock",
      ...(request.authorization.endpoint ? ["--env", "AGENT_PASSPORT_PORT=" + request.authorization.endpoint.port] : []),
      "--env",
      "AGENT_PASSPORT_CREDENTIAL_PATH=/workspace/" + request.authorization.credentialPath.split(/[\\/]/).at(-1),
      "--env",
      "AGENT_PASSPORT_AGENT_ID=" + request.agentId,
      ...(request.authorization.endpoint ? [] : ["--mount", "type=bind,src=" + request.authorization.socketPath + ",dst=/run/agent-passport.sock"]),
    ] : []),
    "--mount",
