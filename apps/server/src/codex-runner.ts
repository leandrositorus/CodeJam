...
export class CodexRunner implements AgentRunner {
  ...
  private childEnvironment(request?: RunnerRequest): NodeJS.ProcessEnv {
    ...
    const environment: NodeJS.ProcessEnv = {
      CODEX_HOME: request?.codexHomePath ?? this.config.codexHome,
    ...
    if (request?.authorization) {
      environment.AGENT_PASSPORT_SOCKET = request.authorization.socketPath;
      environment.AGENT_PASSPORT_CREDENTIAL_PATH = request.authorization.credentialPath;
      environment.AGENT_PASSPORT_AGENT_ID = request.agentId;
    }
    return environment;
  }
}
