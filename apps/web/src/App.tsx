...
export default function App() {
  ...
  const createUser = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!/^[a-zA-Z0-9_.-]{3,40}$/.test(newUsername.trim())) {
      setError("Use a username with 3–40 letters, numbers, underscores, dots or hyphens, such as fyp_owner. Spaces are not allowed.");
      return;
    }
    setBusy(true);
    setError(null);
  ...
  const saveResource = async (event: React.FormEvent) => {
    ...
    setError(null);
    try {
      const sourceAgentId = resourceForm.sourceAgentId;
      if (!sourceAgentId) {
        setError("Select the Agent whose completed chat contains this resource.");
        return;
      }
      const body = { category: resourceForm.category, label: resourceForm.label, sourceAgentId, description: resourceForm.content, sharingEligible: resourceForm.sharingEligible, offerDescriptor: resourceForm.offerDescriptor };
  ...
  const editResource = async (id: string) => { setBusy(true); setError(null); try { const { resource } = await api.resource(id); const detail: ProtectedResourceDetail = resource; setEditingResourceId(id); setResourceForm({ category: detail.category, label: detail.label, sourceAgentId: detail.sourceAgentId ?? "", description: detail.description, content: detail.description, sharingEligible: detail.sharingEligible, offerDescriptor: detail.offerDescriptor }); } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); } finally { setBusy(false); } };
          ...
          <>
            ...
            <section className="playground">
              ...
              <div className="messages">
                ...
                {authorizationDecisions.length > 0 && <section className="authorization-evidence" aria-label="Authorization evidence"><strong>Authorization evidence</strong>{authorizationDecisions.map((decision) => <article className={`run-error ${decision.result === "allow" ? "authorization-allowed" : "authorization-denied"}`} key={decision.id}><span>{decision.result === "allow" ? "Allowed" : "Denied"} — {decision.action} {decision.resourceLabel ?? "resource"}: {decision.reasonCode}</span></article>)}</section>}
                <div ref={messageEnd} />
              </div>
        ...
        <div className="modal-backdrop" onMouseDown={() => setShowUsers(false)}>
          <section className="modal users-modal" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <form className="user-create-form" onSubmit={createUser}>
              <input aria-label="Username" aria-describedby="username-help" placeholder="e.g. fyp_owner" value={newUsername} onChange={(event) => setNewUsername(event.target.value)} minLength={3} maxLength={40} required />
            ...
            <p id="username-help">Username: 3–40 letters, numbers, underscores, dots or hyphens. No spaces. Use fyp_owner and profile_owner for the demo.</p>
            {error && <div className="error-banner" role="alert">{error}</div>}
