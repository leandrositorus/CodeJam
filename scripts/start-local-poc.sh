...
log "Checking that the Runtime can bind-mount an isolated Agent Codex home."
mkdir -p "$CODEX_HOME/agents/.preflight"
...
if ! "$engine" run --rm \
  ...
  --mount "type=bind,src=$CODEX_HOME/agents/.preflight,dst=/codex-home" \
  "$runtime_image" sh -lc \
  ...
  log "The container engine cannot mount $local_state_root/agents/.preflight."
